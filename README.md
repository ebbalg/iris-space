...
1. The chatbot keeps little context of previous conversations. We could save each prompt and output (conversation) and feed it again to the LLM so that it has more context of previous responses. This could be done dynamically so it keeps a certain length of conversation.
